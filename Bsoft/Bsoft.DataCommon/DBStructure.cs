...
namespace Bsoft.Data
{
    ...
    {
        //Dhiraj
        // public DataBaseEngine dbengine = DataBaseEngine.SQLServer;

        #region Member variables.

        private DBConnect _con = null;
        ...
        private bool doescolumnExistSQLite(string tableName, string columnName, string dataType, int dataLength, int dataPrecision, int dataScale)
        {
            ...
            if ((table != null) & (table.Rows.Count > 0))
            {
                ...
                {
                    if (DBConnect.GetStringData(row, "name").ToUpper() == columnName.ToUpper())
                    {
                        str = str.Trim();
                        return ((str.Length == 0) || (DBConnect.GetStringData(row, "type") == str));
                    }
                }
            }
            return false;
