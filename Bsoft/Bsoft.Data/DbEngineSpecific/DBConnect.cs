...
namespace Bsoft.Data
{
    ...
    public class DBConnect : IDisposable
    {
        ...
        private System.Data.SQLite.SQLiteConnection _connection = new SQLiteConnection();
        ...
        {
            commandText = ConversionSql.changeSQLScalar(commandText);
        ...
        {
            commandText = ConversionSql.changeSQLNonQuery(commandText);
            //if application is exiting the don't perform DB process
            if (this.isAppClosing)
            {
                return -1;
            ...
            catch (Exception ex)
            {
                ...
                switch (ConversionSql.dbKind)
                {
                    ...
                    case databaseKind.Oracle:
                        ...
                        {
                            throw new Exception("The data is used in other table so cannot delete it");
                        }
        ...
        /// <param name="commandText">the CommandType (stored procedure, text, etc.)</param>
        /// <returns>Returns a dataReader.</returns>
        public SQLiteDataReader ExecuteDataReader(StringBuilder commandText)
        {
        ...
        /// <summary>
        /// Creates an SQLiteCommand and returns the object.
        /// </summary>
        ...
        public void CommitTransaction(ref SQLiteTransaction pTransaction)
        {
            //if application is exiting the don't perform DB process
            if (this.isAppClosing)
            {
        ...
        /// </summary>
        /// <param name="trans">The System.Data.SQLite.SQLiteTransaction object representing the new transaction.</param>
        public void RollbackTransaction(ref SQLiteTransaction pTransaction)
        {
            //if application is exiting the don't perform DB process
            if (this.isAppClosing)
            {
        ...
        // Parameters:
        // -command - the SQLiteCommand to be prepared
        ...
        private SQLiteCommand PrepareCommand(CommandType commandType, string commandText, SqlParameter[] commandParameters)
        {
            //if application is exiting the don't perform DB process
            if (this.isAppClosing)
            {
        ...
        private void AttachParameters(SQLiteCommand command, SqlParameter[] commandParameters)
        {
        ...
        public static void DisposeObject(SQLiteDataReader dataRdr)
        {
            if ((dataRdr != null))
            {
                dataRdr.Close();
        ...
        public static void DisposeObject(SQLiteCommand cmd)
        {
            if ((cmd != null))
            {
        ...
        {
            return ExecuteScalar("select count(*) from {0}".With(tablename)).ToInt();
        }
