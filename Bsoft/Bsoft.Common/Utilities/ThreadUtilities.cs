...
namespace Bsoft.Threading
{
    public static class ThreadUtilities
    {
        ...
        private delegate void SetProgressCallback(Form f, ProgressBar ctrl, Label lbl, int max, int current);
        ...
        public static void SetProgress(Form form, ProgressBar progressBar1, Label lbl, int Maximum, int current)
        {
            // InvokeRequired required compares the thread ID of the
            // calling thread to the thread ID of the creating thread.
            // If these threads are different, it returns true.
            if (progressBar1.InvokeRequired)
            {
                ...
                form.Invoke(d, new object[] { form, progressBar1, lbl, Maximum, current });
            }
            else
            {
                ...
                if (progressBar1.Value + current <= progressBar1.Maximum)
                {
                    ...
                    lbl.Text = "Download on progress";
                }
                else
                {
                    lbl.Text = "Download on completed";
                }
